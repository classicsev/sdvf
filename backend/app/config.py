...
class Settings(BaseSettings):
    ...
    # СДВФ -> Учёт: узкий server-to-server API только для чтения банковских
    # поступлений при подготовке актов сверки. Это отдельный секрет: ключ
    # генерации документов и SSO-секреты здесь намеренно не переиспользуются.
    sdvf_reconciliation_api_key: str = ""
